...
def tf_logger(logdir, suffix, out_file=None):
    # Set custom logger
    log_dir = Path(logdir)
    ...
    if out_file:
        log_file = log_dir / out_file
        with_time = False
    else:
        log_file = log_dir / suffix
        with_time = True
    # tf_logging._logger = create_logger(log_file=log_file, with_time=with_time, file_level=1,
    #                                    clear_exist_handlers=True, name="tensorflow")
    logger = create_logger(log_file=log_file, with_time=with_time, file_level=1,
                           clear_exist_handlers=True, name="tf")
